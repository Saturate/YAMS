...
This enables **client-side compression**: the LLM reads its accumulated observations via `get_uncompressed_observations`, writes a structured summary, and stores it via `compress_observations`. No server-side LLM needed. See [Compression modes](/docs/configuration#compression-modes).
